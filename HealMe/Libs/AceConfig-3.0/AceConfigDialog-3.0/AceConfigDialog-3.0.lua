...
function AceConfigDialog:Open(appName, container, ...)
	...
	if type(container) == "table" and container.type == "BlizOptionsGroup-Z" and #path > 0 then
	 ...
	 -- localisation of scoped variables
	local status, saved_scrollvalue, saved_offset
	...
	if container then
		...
		status = AceConfigDialog:GetStatusTable(appName)	-- localised earlier
		saved_scrollvalue, saved_offset = status.scrollvalue, status.offset
		if not status.width  then status.width  = 700 end
		if not status.height then status.height = 500 end
	...
	if status and saved_scrollvalue ~= nil then
		status.scrollvalue = saved_scrollvalue
		status.offset	  = saved_offset
		if f.FixScroll then f:FixScroll() end
	end
